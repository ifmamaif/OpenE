 ...
 void SceneObject::Load()
{
 ...
 void SceneObject::BindFull(Camera * const & eye)
 {
	 ...
	 {
		 if (!lightPtrs[j]->isPointLight())
			 lightPtrs[j]->lightDepthFrameBuffer.UseDepthTexture(i + 6 * j);
		 else
			 for (t = 0; t < 6; t++)
				 lightPtrs[j]->lightDepthFrameBuffer.UseDepthTexture(i + 6 * j + t, t);
	 }
 ...
 void TerrainObject::LoadModel(ResourceManager * const &ReqResources)
{
 ...
 void TerrainObject::Key(unsigned char const & key, Camera * const & eye)
  ...
  void LightObject::updateTarget(Camera * const & cameraPtr)
  {
	  if (type == light_type::directional)
	  {
		  lightPOV[0]->position = cameraPtr->normalized_target-direction*12000;
		  lightPOV[0]->target = cameraPtr->normalized_target;
		  lightPOV[0]->updateWorldView();
	  }
  }
  ...
  void LightObject::SendTo(Shader * const & shaderPtr, int const & i)
  {
	  ...
	  if(!isPointLight())
		shaderPtr->SendLightViewProjectionMatrix(&lightPOV[0]->viewProjectionMatrix.m[0][0], i*6);
	  else
	  {
		  for(int j=0;j<6;j++)
		  shaderPtr->SendLightViewProjectionMatrix(&lightPOV[j]->viewProjectionMatrix.m[0][0], i * 6 + j);
	  }
	  ...
	  lightDepthFrameBuffer(true, _type == light_type::point)
  {
	  ...
	  alpha = _alpha;
	  if (type == light_type::directional)
		  lightPOV[0] = new Camera(position-direction*1, position, Vector3(0, 1, 0), 0.1, 0.1, -2000, 2000, -2000, 2000, 0, 15000);
