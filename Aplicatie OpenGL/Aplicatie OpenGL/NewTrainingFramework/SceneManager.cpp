...
void SceneManager::InitLightObjects(xml_node<>* const & pRoot)
{
	...
	for (xml_node<> *Lights = RootLights->first_node(); Lights; Lights = Lights->next_sibling())
	{
		...
		if(_type == light_type::directional)
			lights.try_emplace(id, id, _type, active_cameraPtr->normalized_target, _direction, _light_color, _specular_power, _alpha);
		else
...
void SceneManager::Draw()
{
	if (draw_state == draw_type::DRAW_FULL)
	{
		...
		for (auto &x : lights)
		{	
			x.second.lightDepthFrameBuffer.BindForRendering();
		...
		/*lights[1].lightDepthFrameBuffer.UseDepthTexture(0);
...
void SceneManager::Key(unsigned char const & key)
{
	...
	else
	{
		...
		for(auto &x : lights)
			x.second.updateTarget(active_cameraPtr);
		for (auto &x : objects)
