...
void Camera::updateWorldView()
{
	...
	normalized_target = position - zAxis*(cam_near + cam_far) / 2;
...
Camera::Camera(Vector3 const & position, Vector3 const & target, Vector3 const & up, GLfloat const & moveSpeed, GLfloat const & rotateSpeed, GLfloat left, GLfloat right, GLfloat top, GLfloat bottom, GLfloat cam_near, GLfloat cam_far)
{
	this->position = position;
	this->target = target;
	this->up = up;
	...
	this->cam_near = cam_near;
	this->cam_far = cam_far;
	projectionMatrix.SetOrtho(left, right, top, bottom, cam_near, cam_far);
	updateWorldView();
}
